const app = getApp();
import {formalFixDrugstoredt} from '../../config/api'
...
Page({
  ...
  async _formalFixDrugstoredt() {
    let result = await formalFixDrugstoredt(this.data.id)
    ...
    if(result.data.code === 0){
      ...
      this.setData({
        ...
        [content]:result.data.data.organName,
        longitude: result.data.data.longitude,
        latitude: result.data.data.latitude,
      })
    }else{
  ...
  //纠错
  tojc() {
    my.navigateTo({ url: '/pages/errorCorrection/errorCorrection?type=3&id='+this.data.id})
  },
  onShareAppMessage() {
    return {
      path: '/pages/drugstoreDetail/drugstoreDetail?id='+ this.data.id
    };
  },
});
